...
/** Token-serial top-1 MoE-lite with two experts. */
class MoELiteMini(lanes: Int = 4, numExperts: Int = 2, dataWidth: Int = 8, accWidth: Int = 32) extends Module {
  require(numExperts == 2, "MoELiteMini first implementation supports exactly 2 experts")
  ...
  val router = Module(new RouterMini(lanes, numExperts, dataWidth, accWidth))
  router.io.x := io.x
  router.io.weight := io.routerWeight
  router.io.bias := io.routerBias

  val experts = Seq.fill(numExperts)(Module(new ExpertMLPMini(lanes, dataWidth, accWidth)))
