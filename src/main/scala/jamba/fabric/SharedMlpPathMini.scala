...
/** Dense-or-MoE MLP path with both branches mapped onto shared fabric implementations. */
class SharedMlpPathMini(lanes: Int = 4, numExperts: Int = 2, dataWidth: Int = 8, accWidth: Int = 32) extends Module {
  require(lanes == 4, "SharedMlpPathMini currently uses SharedDenseMLPMini and requires lanes == 4")
  require(numExperts == 2, "SharedMlpPathMini first MoE-lite implementation supports exactly 2 experts")
  ...
  val moe = Module(new SharedMoELiteMini(lanes, numExperts, dataWidth, accWidth))
  moe.io.x := io.x
