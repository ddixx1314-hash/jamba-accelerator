...
class Jamba2MiniLayerSpec extends AnyFlatSpec with ChiselScalatestTester {
  behavior of "Jamba2MiniLayer"
  ...
  it should "run the formal Mixer plus MLP layer in Attention mode" in {
    test(new Jamba2MiniLayer()) { dut =>
      dut.io.en.poke(true.B)
      dut.io.clear.poke(false.B)
