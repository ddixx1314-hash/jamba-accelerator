...
class SerialJamba2MiniLayerSpec extends AnyFlatSpec with ChiselScalatestTester {
  behavior of "SerialJamba2MiniLayer"

  // Expected values match SharedJamba2MiniLayerSpec (same semantics, serial timing).
  ...
  private def pokeExpertWeights(dut: SerialJamba2MiniLayer): Unit = {
  ...
  private def pokeDefaultWeights(dut: SerialJamba2MiniLayer): Unit = {
  ...
  private def runToDone(dut: SerialJamba2MiniLayer, maxCycles: Int = 200): Unit = {
    var seenDone = false
    ...
    assert(seenDone, s"SerialJamba2MiniLayer did not finish within $maxCycles cycles")
  }

  it should "run Mamba mode and match SharedJamba2MiniLayer second-cycle output" in {
    // Serial layer computes the full pipeline in one invocation.
    // Expected: same values as SharedJamba2MiniLayer after clock.step() in Mamba mode.
    // mixerY=[2,0,0,0], firstResidual=[3,0,0,0], mlpY=[0,0,0,1], y=[3,0,0,1]
    test(new SerialJamba2MiniLayer()) { dut =>
  ...
  it should "run Attention mode and write KV cache" in {
    // x=[4,0,0,0]; norm1 divides by mean_square=4 → norm1_y=[1,0,0,0]
    // attention: Q=K=V=[1,0,0,0]; score[0]=1, weight[0]=0; rawY=[0,0,0,0]; y=[0,0,0,0]
    // firstResidual=[4,0,0,0]; norm2 → [1,0,0,0]; mlpY=[0,0,0,1]; y=[4,0,0,1]
    test(new SerialJamba2MiniLayer()) { dut =>
      dut.io.clear.poke(false.B)
  ...
  it should "toggle between Mamba and Attention modes on consecutive tokens" in {
    test(new SerialJamba2MiniLayer()) { dut =>
      dut.io.clear.poke(false.B)
      dut.io.enableMoE.poke(false.B)
      pokeDefaultWeights(dut)
  ...
  it should "clear resets all state" in {
    test(new SerialJamba2MiniLayer()) { dut =>
      dut.io.clear.poke(false.B)
      dut.io.useAttention.poke(false.B)
      dut.io.enableMoE.poke(false.B)
