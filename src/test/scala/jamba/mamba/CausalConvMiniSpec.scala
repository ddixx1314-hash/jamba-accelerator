...
class CausalConvMiniSpec extends AnyFlatSpec with ChiselScalatestTester {
  behavior of "CausalConvMini"
  ...
  it should "compute a four-tap causal convolution over time" in {
    test(new CausalConvMini(lanes = 4, taps = 4)) { dut =>
      dut.io.en.poke(true.B)
      dut.io.clear.poke(false.B)
      pokeKernel(dut.io.kernel, Seq.fill(4)(Seq.fill(4)(1)))
      ...
      expectVector(dut.io.y, Seq(9, 11, 13, 15))
    }
  }

  it should "hold history when disabled and clear history" in {
    test(new CausalConvMini(lanes = 4, taps = 4)) { dut =>
      dut.io.en.poke(true.B)
      dut.io.clear.poke(false.B)
      pokeKernel(dut.io.kernel, Seq.fill(4)(Seq.fill(4)(1)))
