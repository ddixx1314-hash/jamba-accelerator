...
class DotProductSpec extends AnyFlatSpec with ChiselScalatestTester {
  behavior of "DotProduct"

  private def pokeVector(dut: DotProduct, a: Seq[Int], b: Seq[Int]): Unit = {
    for (i <- a.indices) {
