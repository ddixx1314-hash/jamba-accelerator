...
class TinyJambaBlockSpec extends AnyFlatSpec with ChiselScalatestTester {
  behavior of "TinyJambaBlock"
  ...
  private def pokeCommon(dut: TinyJambaBlock): Unit = {
  ...
  it should "emit the Mamba path when attention is disabled" in {
    test(new TinyJambaBlock()) { dut =>
      pokeCommon(dut)
  ...
  it should "add tiny attention output when attention is enabled" in {
    test(new TinyJambaBlock()) { dut =>
      pokeCommon(dut)
