...
This report is the seed for the paper's resource-reuse table. It currently compares direct `DotProduct`, `Linear4`, `AttentionDecodeTiny`, and `CausalConv1D` implementations against their shared-fabric versions. Later reports should add shared Mamba mixer, shared MLP, and tile-level comparisons.
