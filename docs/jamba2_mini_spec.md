...
The first formal accelerator shell is implemented as `Jamba2MiniTile`. It provides token valid/ready IO, `start`/`clear`/`enableMoE`/`useLoadedWeights` command controls, `busy`/`done`/`error` status, debug outputs, and a `WeightStoreMini` load/read shell. The Stage 13 demo runs a two-token trace against `jamba2_mini_tile_demo_trace` and checks output, SSM state, and attention KV cache progress. The tile uses deterministic demo weights by default and can decode loaded weights into the hybrid core for the shared norm, Mamba, attention, dense MLP, and router ports.
