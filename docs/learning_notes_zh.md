...
```text
sumSquares = sum(x(i) * x(i))
meanSquare = floor(sumSquares / length)
```
...
```text
y = x_now * k0 + x_prev * k1 + x_prevprev * k2
```
...
```text
score(row) = dot(q, keys(row))
y(col) = sum(score(row) * values(row)(col))
```
...
```text
RmsNormApprox
 -> input/gate/B/C Linear4 projections
 -> TinyJambaBlock
 -> output Linear4 projection
```
...
```text
fire = inValid && inReady
```

输出消费：

```text
willConsume = outValid && outReady
```
